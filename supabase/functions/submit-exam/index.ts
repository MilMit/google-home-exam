...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    const ansMap=new Map<number,number[]>((answers??[]).map((a:any)=>[Number(a.question_id),(a.selected_option_ids??[]).map(Number)] as [number,number[]])),optMap=new Map<number,any[]>()
    for(const o of opts??[]){const qid=Number(o.question_id);const l=optMap.get(qid)??[];l.push(o);optMap.set(qid,l)}
    let score=0,maxScore=0;const sections:Record<string,{score:number,max:number}>={}
    for(const q of qs??[]){
      const qid=Number(q.id);const correct=(optMap.get(qid)??[]).filter(o=>o.is_correct).map(o=>Number(o.id)),selected=ansMap.get(qid)??[];let earned=0;const max=Number(q.max_points)
    ...
    const {data:events}=await admin.from('attempt_events').select('event_type,event_data,created_at').eq('attempt_id',attempt_id).order('created_at',{ascending:true})
    const ev=events??[]
    ...
    const basePenalty=Math.min(35,tabHidden*10)+Math.min(35,cameraStopped*25)+Math.min(20,fullExit*8)+Math.min(10,offline*2)+Math.min(20,(copy+paste)*4)

    const behavior=analyzeBehavior(ev,{...attempt,submitted_at:new Date().toISOString(),percentage},percentage)
    const totalPenalty=Math.min(100,basePenalty+behavior.penalty)
    const integrityScore=Math.max(0,100-totalPenalty)
    const integrityStatus=(integrityScore>=80&&!behavior.critical)?'valid':'review'
    const securitySummary={
      tab_hidden:tabHidden,
      camera_stopped:cameraStopped,
      fullscreen_exit:count(ev,'fullscreen_exit'),
      fullscreen_penalized:fullExit,
      network_offline:offline,
      copy_attempt:copy,
      paste_attempt:paste,
      translation_grace:translationGrace,
